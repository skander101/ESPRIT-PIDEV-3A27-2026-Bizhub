...
public class InvestmentServiceImpl implements IInvestmentService {
    ...
    public int add(Investment investment) throws SQLException {
        String sql = "INSERT INTO investment (project_id, investor_id, amount, contract_url, investment_date) VALUES (?, ?, ?, ?, NOW())";
    ...
    public List<Investment> getAll() throws SQLException {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                Timestamp ts = rs.getTimestamp("investment_date");
                inv.setInvestmentDate(ts != null ? ts.toLocalDateTime() : null);
