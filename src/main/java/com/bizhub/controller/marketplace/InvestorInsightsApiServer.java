...
public class InvestorInsightsApiServer {

    private static HttpServer server;
    private static int activePort = -1;

    private static final Gson gson = new Gson();
    private static final InvestorStatsService statsService = new InvestorStatsService();
    private static final AiInsightsService aiService = new AiInsightsService();

    private static final int PORT_START = 8100;
    private static final int PORT_END   = 8105;
    ...
    public static int start() throws Exception {
        ...
        if (server == null) throw new IllegalStateException("Aucun port libre entre 8100 et 8105");

        server.createContext("/api/investor/insights", (HttpExchange ex) -> {
            try {
                ...
                List<StatsPoint> points = statsService.daily(investorId, from, to);
                InvestorInsightResponse insights = aiService.analyze(points);

                byte[] json = gson.toJson(insights).getBytes(StandardCharsets.UTF_8);
                ex.getResponseHeaders().set("Content-Type", "application/json; charset=utf-8");
                ex.sendResponseHeaders(200, json.length);
                try (OutputStream os = ex.getResponseBody()) { os.write(json); }

            } catch (Exception e) {
                byte[] msg = ("{\"error\":\"" + safe(e.getMessage()) + "\"}").getBytes(StandardCharsets.UTF_8);
                ex.getResponseHeaders().set("Content-Type", "application/json; charset=utf-8");
