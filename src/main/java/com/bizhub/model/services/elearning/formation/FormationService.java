package com.bizhub.model.services.elearning.formation;
...
public class FormationService {
    ...
    public void create(Formation f) throws SQLException {
        String sql = "INSERT INTO formation(title, description, trainer_id, start_date, end_date, cost, lieu, en_ligne) VALUES (?,?,?,?,?,?,?,?)";
    ...
    public Optional<Formation> findById(int formationId) throws SQLException {
        String sql = "SELECT formation_id, title, description, trainer_id, start_date, end_date, cost, lieu, en_ligne FROM formation WHERE formation_id=?";
    ...
    public List<Formation> findAll() throws SQLException {
        String sql = "SELECT formation_id, title, description, trainer_id, start_date, end_date, cost, lieu, en_ligne FROM formation ORDER BY formation_id DESC";
    ...
    public List<Formation> findByTrainerId(int trainerId) throws SQLException {
        String sql = "SELECT formation_id, title, description, trainer_id, start_date, end_date, cost, lieu, en_ligne FROM formation WHERE trainer_id=? ORDER BY formation_id DESC";
        List<Formation> out = new ArrayList<>();
        try (PreparedStatement pst = cnx.prepareStatement(sql)) {
    ...
    public void update(Formation f) throws SQLException {
        String sql = "UPDATE formation SET title=?, description=?, trainer_id=?, start_date=?, end_date=?, cost=?, lieu=?, en_ligne=? WHERE formation_id=?";
        try (PreparedStatement pst = cnx.prepareStatement(sql)) {
            pst.setString(1, f.getTitle());
            pst.setString(2, f.getDescription());
            pst.setInt(3, f.getTrainerId());
    ...
    private static Formation mapRow(ResultSet rs) throws SQLException {
        ...
        f.setEnLigne(rs.getBoolean("en_ligne"));
